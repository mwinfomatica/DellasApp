...
class ContextoServices {
  late ContextoModel contextoModel;

  Future<void> setEnderecoGrupo({bool engerecoGrupo = false}) async {
  ...
  Future<void> setDeviceSelected({String nameDevice = "", String uuidDevice = ""}) async {
    ContextoModel contextoModel = await getContexto();
    if (contextoModel != null) {
      SharedPreferences prefs = await SharedPreferences.getInstance();
  ...
  Future<void> setTipoLeitor({bool leitorexterno = false}) async {
    ContextoModel contextoModel = await getContexto();
    if (contextoModel == null) {
